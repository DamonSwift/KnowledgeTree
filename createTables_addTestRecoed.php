...
<?php
  ...
  if(!$conn) {
    ...
    echo " Connected!</br> Server: $server</br> User: $user </br></br></br>";
  }
  ...
  echo $tabKnowTree . "</br>";
  //输出格式：
  // CREATE TABLE if not exists KnowledgeTree( 
  //     TreeName varchar(50) PRIMARY KEY, 
  //     Domain varchar(50), 
  //     CreateTime date, 
  //     UpdateTime timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, 
  ...
  //添加测试树
  $sql = "INSERT INTO if not exists $tabKnowTree (TreeName, Domain, CreateTime, UpdateTime, Comment)
