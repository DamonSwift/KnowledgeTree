...
var setting = {
  ...
  async: {
    ...
    url        : "PHP/getNodes_MySQL.php" //Ajax 获取数据的 URL 地址
  },
...
function addKnowledgeTree(event) {
  ...
  $.post("PHP/setNewKnowTree_MySQL.php", newTree);
};
...
function addKnowledgeNode(event) {
  ...
  if(!treeNode) { 
    ...
    $.post('PHP/getNewNodeID_MySQL.php', { id: treeNode.id, }, function(data){
      var newNode = {
        id: "",
        pId: treeNode.id,
        isParent: "false",
    ...
    zTree.editName(treeNode[0]);
    // $.post('PHP/setNewKnowNode_MySQL.php', param1: 'value1', function(data, textStatus, xhr) {
...
function remove(e) {
  ...
  $.post('PHP/removeNode_MySQL.php', { id: treeNode.id, clearChildrenOnly: false });
...
function clearChildren(e) {
  ...
  $.post('PHP/removeNode_MySQL.php', { id: treeNode.id, clearChildrenOnly: true });
...
function reset() {
  ...
  //后台数据库重置(同步AJAX)
  $.ajax({
    url: 'PHP/createTables_addTestRecoed.php',
    async: false,
